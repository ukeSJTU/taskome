...
export E2E_WEB_URL="http://127.0.0.1:$web_port" E2E_GATEWAY_URL="http://127.0.0.1:$gateway_port"
...
export BETTER_AUTH_URL="$E2E_WEB_URL" AUTH_TRUSTED_ORIGIN="$E2E_WEB_URL" GATEWAY_PUBLIC_URL="$E2E_GATEWAY_URL" GATEWAY_INTERNAL_URL="$E2E_GATEWAY_URL"
...
cleanup() { status=$?; if [[ $status -ne 0 ]]; then docker compose -p "$project" -f infra/e2e/compose.yml logs >"$logs_dir/compose.log" 2>&1 || true; fi; docker compose -p "$project" -f infra/e2e/compose.yml down --volumes --remove-orphans || true; exit "$status"; }
