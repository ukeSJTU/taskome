...
Today, that "everything else" is exactly one table: `input_files` (`id`, `owner_user_id`, `original_filename`, `created_at`, `deleted_at`).

> **Status note (delete once built):** Gateway has no `jobs` or `tasks` table yet — this is the same gap [`containers.md`](./containers.md) flags for Job dispatch. Once that lands, this page needs a Job data domain alongside Input Files.
