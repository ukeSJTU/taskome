...
**Database instrumentation trigger:** add the official SQLAlchemy instrumentation with the first real Job or Input File query slice, centrally at engine creation. It must exclude URLs, credentials, bind values, and user input; avoid readiness-check span noise; and verify safe business-query spans with the in-memory exporter. It is intentionally absent from the persistence foundation.
