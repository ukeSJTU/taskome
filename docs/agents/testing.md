...
**better-auth `testUtils`**: `packages/auth/src/test.ts`'s `createTestAuth()` registers the `testUtils({ captureOTP: true })` plugin, exposing `ctx.test` helpers (`createUser`, `saveUser`, `login`, `getAuthHeaders`, `getOTP`). `test/auth.ts`'s `useTestAuth()` mounts a fresh instance's `.handler` behind MSW, verified working for plain `fetch()` callers. **Known gap**: `@/lib/auth-client`'s `authClient` (better-auth's browser client) does not route its requests through MSW or even a direct `vi.stubGlobal("fetch", ...)` in this jsdom+Vitest setup — the request bypasses both and hits a real socket (ECONNREFUSED). Root cause not yet found; until it is, test `login-form`/`signup-form` (and anything else calling `authClient`) by mocking `@/lib/auth-client` at the module boundary instead of through `useTestAuth()`.
