...
Before taking the exception, compare the intended target files with the existing diff and preserve unrelated changes. Continue from overlapping changes only when the user's request or branch history makes their ownership clear; otherwise report the concrete overlap or dependency and wait for direction.
