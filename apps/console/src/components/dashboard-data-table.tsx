...
import {
  ...
  ChartTooltipContent,
  type ChartConfig,
} from "@taskome/ui/components/chart";
...
import {
  ...
  DrawerTitle,
  DrawerTrigger,
} from "@taskome/ui/components/drawer";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { useIsMobile } from "@taskome/ui/hooks/use-mobile";
...
import { Separator } from "@taskome/ui/components/separator";
import { TableCell, TableRow } from "@taskome/ui/components/table";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@taskome/ui/components/tabs";
import {
  CircleCheckIcon,
...
const columns = columnHelper.columns([
  ...
  columnHelper.accessor("status", {
    ...
    cell: ({ row }) => (
      <Badge variant="outline" className="px-1.5 text-muted-foreground">
        {row.original.status === "Done" ? <CircleCheckIcon /> : <LoaderIcon />}
        {row.original.status}
      </Badge>
    ),
  }),
...
export function DashboardDataTable({ data: initialData }: { data: z.infer<typeof schema>[] }) {
  ...
  return (
    <DataTable.Root table={table}>
      <Tabs defaultValue="outline" className="w-full flex-col justify-start gap-6">
        <DataTable.Toolbar className="px-4 lg:px-6">
          ...
          >
            <SortableContext items={dataIds} strategy={verticalListSortingStrategy}>
              <DataTable.Content
                ...
                empty={<DataTable.Empty className="h-24 min-h-24 p-0">No results.</DataTable.Empty>}
