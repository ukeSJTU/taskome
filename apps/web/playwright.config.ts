...
export default defineConfig({
  ...
  webServer: [
    {
      command: production
        ? `pnpm build && mkdir -p .next/standalone/apps/web/.next .next/standalone/apps/web/public && cp -R .next/static .next/standalone/apps/web/.next/static && cd .next/standalone && HOSTNAME=127.0.0.1 PORT=${webPort} node apps/web/server.js`
